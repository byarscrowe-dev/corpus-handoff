...
| 2026-06-06 | Phase 2 CONGRESS family built + DEPLOYED (forward-only) — 9 bots: congress_all/leadership/active + 6 mimics (Pelosi/Wyden/Crenshaw/Wasserman Schultz/Gottheimer + Rouzer passive control); stock_ingest.py House eFD + Senate ingest (digital-PDF only, append-only c_congress_trades, atomic congress_signals.json); pure CongressionalCopyStrategy (disclosure-date no-lookahead, held-skip incl. in-flight pending) + CONGRESSIONAL_DYNAMIC universe (held-union); engine additive-only (import + _STRATEGY_MAP + run_backtest guard); systemd ingest timer 5:15 PM CT; test_step6.py (61) → 290 tests green; ultra-planned + adversarially reviewed (6 findings fixed). LIVE-VERIFIED: pdfplumber 0.11.9 installed, ingest ran clean (House 234 docs, Senate 8350, 5182 signals, 0 errors), health OK; coverage fix c359447 (skip already-ingested House DocIDs past the per-run PDF cap). Tradeable mimic data: Gottheimer 31 / Pelosi 5 / Rouzer 2; Wyden munis-only (idle); Crenshaw + Wasserman Schultz no 2026 filings yet. First forward snapshots Mon 6 PM CT | 26f9cec, c359447 |
| 2026-06-07 | CONGRESS leadership roster finalized (strict equity-trader roster) — removed Ron Wyden from congress_leadership.csv (munis/non-equity only → zero tradeable signal); roster now Capito (C001047) / Whitehouse (W000802) / French Hill (H001072, with "Hill James French" aliases) / Roger Williams (W000816); companion alias rows added so filings resolve to Bioguide IDs not NM:<slug>; mimic_wyden alias preserved (that mimic unaffected). Grassley (G000386) evaluated & EXCLUDED — files no individual-equity PTRs (mutual funds only); NOT added to dead_signals (that's for departed filers). test_step6.py 61 passed (temp DB; live dashboard.db untouched). POST-INGEST VERIFICATION PENDING: after the next 5:15 PM CT ingest, confirm the 4 leaders resolve to Bioguide IDs (not NM:<slug>). | be828e6 |
...
| Phase 2 CONGRESS family is forward-only with a pure-strategy / ingest-owns-I/O split; engine stays frozen | Copying disclosed congressional trades carries a ~45-day reporting lag and dollar-RANGE amounts, so a backtest is hindsight-contaminated and exact sizing is impossible — all 9 bots are `forward_only=True`/`backtestable=False`, fixed 10% sizing, 7% stop, graded forward vs SPY. Architecture keeps the frozen engine untouched: a SEPARATE ingest job (systemd 17:15 CT, not APScheduler — OOM safety on the 2GB VPS) owns ALL network + DB writes into append-only `c_congress_trades` + an atomically-written `congress_signals.json`; `CongressionalCopyStrategy` reads ONLY that cache (no DB/network — pure), gates on `disclosure_date <= today` (no-lookahead) within a 5-day lookback, mirrors buys+sells, and skips held + in-flight pending tickers (the engine has no pending-order dedup, so ingest bakes pending BUYs into the held set). Universe `CONGRESSIONAL_DYNAMIC` = disclosed(recency-capped) ∪ uncapped-held, returns [] never raises. Members keyed by Bioguide ID (alias CSV, word-order-insensitive); dead signals (Green/Bucshon) excluded via CSV. Free data: House Clerk eFD + Senate JSON, digital PDFs only (OCR deferred). Built via an ultra-plan workflow + adversarial code review (6 findings fixed); 290 tests green. | 2026-06-06 |
...
2. ✅ **DONE 2026-06-06** — Phase 2 CONGRESS family BUILT + DEPLOYED + live-verified (9 forward-only bots; ingest live: House 234 docs + Senate, 5182 signals, 0 errors, health OK; pdfplumber 0.11.9 installed; ingest timer enabled 5:15 PM CT).
3. **Mon 2026-06-08 check** — confirm the 5:15 PM ingest fired, then all 9 congress bots wrote a forward snapshot at the 6 PM run (their first real forward day). `congress_leadership` + `mimic_crenshaw` + `mimic_wasserman_schultz` may be idle until they file / the roster expands — expected, not a failure. Watch the health monitor's `congress_ingest` WARN (gap-ratio / parser-drift canary).
4. **House parser:** live digital-PDF parse works (168+ tradeable rows first run). OCR for scanned filings (~47 gaps/run) is a deferred follow-up; revisit only if gap-ratio matters.
5. **CONGRESS leadership roster — verification PENDING.** Strict roster finalized (be828e6): Capito / Whitehouse / French Hill / Roger Williams, Wyden removed (munis-only), companion aliases added. After the next 5:15 PM CT ingest, confirm these 4 resolve to their Bioguide IDs (not `NM:<slug>`) and that `congress_leadership` picks up whoever actually files individual-equity PTRs. Re-verify/expand the roster each new Congress.
6. Multi-horizon robustness panel design session — once meaningful forward data accumulates (weeks/months)
7. Future chaos exploration: churn-rate ladder (sell_prob never/10%/30%) — pure-randomness variable isolation
